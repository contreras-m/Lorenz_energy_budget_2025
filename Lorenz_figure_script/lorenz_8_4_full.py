...
def plot_lorenz_energy_cycle(mvar):
    ...
    # Conversion arrows
    draw_doublearrow( r'$BKE^{8km}$',  r'$SMKE^{8km}$', r'$CK_{(SM,B)}^{8km}$' + '\n' + fr'$({mvar[3]:.2f})$',dimx_box,dimy_box,arrowdir='->', side="right", offset=(-0.1, 0),sizefont=12)
    draw_doublearrow(r'$BKE^{8km}$', r'$SMKE^{8km}$', r'$CK_{(B,SM)}^{8km}$' + '\n' + fr'$( {mvar[2]:.2f})$',dimx_box,dimy_box,arrowdir='->', side="left", offset=(-1, 0),sizefont=12)
    draw_varrow( r'$BKE^{8km}$',r'$SMKE^{8km}$', r'$BnK_{(B,SM)}^{8km}$' + '\n' + fr'$({mvar[3]+mvar[2]:.2f})$',dimx_box,dimy_box,arrowdir='->', offset=(0, 1),sizefont=12)

    draw_doublearrow(r'$BPE^{8km}$', r'$SMPE^{8km}$', r'$CP_{(SM,B)}^{8km}$' + '\n' + fr'$({mvar[10]:.2f})$',dimx_box,dimy_box, side="right", offset=(-0.1, 0),sizefont=12)
    draw_doublearrow(r'$BPE^{8km}$', r'$SMPE^{8km}$', r'$CP_{(B,SM)}^{8km}$' + '\n' + fr'$( {mvar[9]:.2f})$',dimx_box,dimy_box, side="left", offset=(-1, 0),sizefont=12)
    draw_varrow(r'$BPE^{8km}$', r'$SMPE^{8km}$', r'$BnP_{(B,SM)}^{8km}$' + '\n' + fr'$({+mvar[10]+mvar[9]:.2f})$',dimx_box,dimy_box,sign="positive", offset=(0, 1),sizefont=12)

    draw_arrow(r'$SMPE^{8km}$', r'$SMKE^{8km}$', fr'$PK_{{SM}}^{{8km}} \ ({mvar[6]:.2f})$' ,dimx_box,dimy_box,arrowdir='->', direction="v", offset=(0, 0),sizefont=18)
    draw_arrow( r'$BPE^{8km}$',  r'$BKE^{8km}$', fr'$PK_{{B}}^{{8km}} \ ({mvar[11]:.2f})$',dimx_box,dimy_box, direction="v",arrowdir='->', offset=(0, 0),sizefont=18)

    draw_residual( r'$BKE^{8km}$',r'$FK_B^{8km}+DK_B^{8km}$' + fr'$({mvar[24]:.2f})$',dimx_box,dimy_box,lines=3, offset=(0,0),sizefont=12)
    draw_residual(r'$SMKE^{8km}$',r'$FK_{SM}^{8km}+DK_{SM}^{8km}$'+ fr'$({mvar[22]:.2f})$',dimx_box,dimy_box,lines=3,arrowdir='->', offset=(0,0),sizefont=12)



    draw_onebox(r'$SMKE^{8km}$', fr'$P_{{SM}}^{{8km}} \ ({mvar[7]-mvar[6]:.2f})$',dimx_box,dimy_box,location="bottom",arrowdir='<-'  ,position=(-0.2, -.8), offset=(0.7, 0.15),sizefont=12)
    draw_onebox(r'$SMKE^{8km}$', fr'$AK_{{SM}}^{{8km}} \  ({mvar[0]:.2f})$',dimx_box,dimy_box,location="bottom",arrowdir='->' ,position=(+1.8, -0.8), offset=(0.7, 0.15),sizefont=12)

    draw_onebox(r'$BKE^{8km}$',fr'$P_{{B}}^{{8km}} \ ({mvar[8]-mvar[11]:.2f})$',dimx_box,dimy_box,location="bottom",arrowdir='<-'  ,position=(-0.2, -.8), offset=(0.7, 0.15),sizefont=12)
    draw_onebox(r'$BKE^{8km}$',fr'$AK_{{B}}^{{8km}} \ ({mvar[1]:.2f})$',dimx_box,dimy_box,location="bottom",arrowdir='<-'  ,position=(+1.8, -0.8), offset=(0.7, 0.15),sizefont=12)

    draw_onebox(r'$SMPE^{8km}$', fr'$AP_{{SM}}^{{8km}} \  ({mvar[4]:.2f})$',dimx_box,dimy_box,location="upper",arrowdir='<-'  ,position=(+1.8, 1.5), offset=(0.7, 0.15),sizefont=12)
    draw_onebox( r'$BPE^{8km}$', fr'$AP_{{B}}^{{8km}} \ ({mvar[5]:.2f})$',dimx_box,dimy_box,location="upper" ,arrowdir='<-',position=(+1.8, 1.5), offset=(0.7, 0.15),sizefont=12)

    draw_residual(r'$BPE^{8km}$',r'$FP_B^{8km}+DP_B^{8km}$'+fr'$ ({mvar[25]:.2f})$',dimx_box,dimy_box,lines=2, offset=(0,0),sizefont=12)
    draw_residual(r'$SMPE^{8km}$',r'$FP_{{SM}}^{8km}+DP_{{SM}}^{8km} $'+fr'$ ({mvar[23]:.2f})$',dimx_box,dimy_box,lines=2, offset=(0,0),sizefont=12)
    ax.text(1, 10 , '(b) 8 KM', ha='center', va='center', fontsize=22,fontweight='bold')

    
    plt.savefig('/Users/contrema/Documents/lorenz_8__full.png',dpi=200, bbox_inches='tight')
